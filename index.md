...
The [`r2()`](https://indenkun.github.io/kvr2/reference/r2_kvr2.md)
function returns a list object. While the output is formatted for
readability, you can easily access individual values for further
...
to evaluate models via standard error metrics such as RMSE, MAE, and
MSE.
...
function allows you to instantly see the impact of the intercept
constraint. In the example below, notice how \\R^2_2\\ remains
misleadingly high in the no-intercept model, whereas \\R^2_1\\ drops,
reflecting the true decrease in predictive accuracy relative to the
mean.
...
certain \\R^2\\ definitions fail when an intercept is removed. By
calling [`plot()`](https://rdrr.io/r/graphics/plot.default.html) on a
`comp_model` object, you generate a comprehensive **2x2 Diagnostic
Dashboard**.
...
- **R-squared Comparison (Top-Left)**: Side-by-side bars for all 9
  definitions. Orange bars instantly flag “illegal” values (e.g., \\R^2
  \> 1\\ or \\R^2 \< 0\\), which are common in no-intercept models.
